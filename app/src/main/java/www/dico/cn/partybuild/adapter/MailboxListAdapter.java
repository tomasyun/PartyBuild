...
public class MailboxListAdapter extends CommonAdapter<MailboxListBean.DataBean> {
    ...
    protected void convert(ViewHolder holder, MailboxListBean.DataBean mailboxListBean, int position) {
        GlideUtils.loadCircleImage(AppManager.getManager().findActivity(MailboxListActivity.class), AppConfig.urlFormat(mailboxListBean.getAvatar()), (ImageView) holder.getView(R.id.iv_avatar_mailbox_item));
